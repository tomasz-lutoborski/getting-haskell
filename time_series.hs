...
median xs = median' sorted
  where
    sorted = sort xs
    len = length xs
    median' xs
      | odd len = realToFrac $ xs !! mid
      | otherwise = avg $ take 2 $ drop (mid - 1) xs
    mid = len `div` 2
    avg [x, y] = (realToFrac x + realToFrac y) / 2

medianTS :: (Real a) => TS a -> Maybe Double
medianTS (TS _ []) = Nothing
medianTS (TS _ values) = if all (== Nothing) values then Nothing else Just med
  where
    med = median (cleanVals values)

cleanTS :: (Real a) => TS a -> [a]
cleanTS (TS _ []) = []
cleanTS (TS _ values) = if all (== Nothing) values then [] else cleanVals values

cleanVals :: (Real a) => [Maybe a] -> [a]
cleanVals [] = []
cleanVals (Nothing : xs) = cleanVals xs
cleanVals (Just x : xs) = x : cleanVals xs
...
makeTSCompare func = newFunc
  where
    ...
    newFunc (_, Nothing) (i, value) = (i, value)
    newFunc (i, value) (_, Nothing) = (i, value)
...
movingAvg values n =
  if length nextValues == n
    then meanMaybe nextValues : movingAvg restValues n
    else []
  where
